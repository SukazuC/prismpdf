...
<h1>Contact sheet — ${vp} (${files.length} screenshots)</h1>
<div class="grid">`;

  for (const f of files) {
    const relPath = path.posix.join(".", vp, f);
    html += `<div class="card"><img src="${relPath}" alt="${f}"><div class="label">${f}</div></div>`;
  }
...
// All-viewport contact sheet
let allHtml = `<!DOCTYPE html><html><head><meta charset="utf-8">
<style>
* { margin: 0; padding: 0; box-sizing: border-box; }
body { background: #0a0a1a; color: #e0e0e0; font-family: monospace; padding: 20px; }
h2 { font-size: 18px; margin: 24px 0 12px; color: #a0a0ff; border-top: 1px solid #2a2a4a; padding-top: 20px; }
.grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 12px; }
.card { background: #14142a; border: 1px solid #2a2a4a; border-radius: 6px; overflow: hidden; }
.card img { width: 100%; display: block; }
.card .label { padding: 6px 8px; font-size: 10px; color: #a0a0c0; word-break: break-all; }
</style></head><body>
<h1>Contact sheet — all viewports</h1>`;
...
for (const vp of VIEWPORTS) {
  ...
  for (const f of files) {
    const relPath = path.posix.join(".", vp, f);
    allHtml += `<div class="card"><img src="${relPath}" alt="${f}"><div class="label">${f}</div></div>`;
  }
  allHtml += "</div>";
}
